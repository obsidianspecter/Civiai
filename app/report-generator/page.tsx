...
export default function ReportGeneratorPage() {
  ...
  const generatePDF = async () => {
    try {
      ...
      reportDiv.style.width = '420mm' // A3 width (297mm * √2 to maintain proportion)
      reportDiv.innerHTML = `
        <div style="font-family: Arial, sans-serif; color: #000; max-width: 100%;">
          <h1 style="color: #333366; text-align: center; font-size: 32px; margin-bottom: 30px;">
            ${formData.projectTitle}
          </h1>
          
          <div style="margin-bottom: 30px; font-size: 16px;">
            <p style="margin-bottom: 10px;"><strong>Project Type:</strong> ${formData.projectType}</p>
            ${formData.clientName ? `<p style="margin-bottom: 10px;"><strong>Client:</strong> ${formData.clientName}</p>` : ''}
            ${formData.location ? `<p style="margin-bottom: 10px;"><strong>Location:</strong> ${formData.location}</p>` : ''}
          </div>
          
          <div style="margin-bottom: 30px;">
            <h2 style="color: #333366; font-size: 24px; margin-bottom: 15px;">Objective</h2>
            <p style="font-size: 16px; line-height: 1.6;">${formData.objective.replace(/\n/g, '<br>')}</p>
          </div>
          
          <div style="margin-bottom: 30px;">
            <h2 style="color: #333366; font-size: 24px; margin-bottom: 15px;">Materials Used</h2>
            <p style="font-size: 16px; line-height: 1.6;">${formData.materials.replace(/\n/g, '<br>')}</p>
          </div>
          
          <div style="margin-bottom: 30px;">
            <h2 style="color: #333366; font-size: 24px; margin-bottom: 15px;">Methodology</h2>
            <p style="font-size: 16px; line-height: 1.6;">${formData.methodology.replace(/\n/g, '<br>')}</p>
          </div>
          
          <div style="margin-bottom: 30px;">
            <h2 style="color: #333366; font-size: 24px; margin-bottom: 15px;">Results & Findings</h2>
            <p style="font-size: 16px; line-height: 1.6;">${formData.results.replace(/\n/g, '<br>')}</p>
          </div>
          
          <div style="text-align: center; margin-top: 50px; color: #666; border-top: 1px solid #ddd; padding-top: 20px;">
            <p style="font-size: 14px; margin-bottom: 5px;">Generated by CiviAI - Civil Engineering Assistant</p>
